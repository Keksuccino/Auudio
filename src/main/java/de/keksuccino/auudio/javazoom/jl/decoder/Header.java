...
public final class Header {
   ...
   void parseVBR(byte[] firstframe) throws BitstreamException {
      ...
      try {
         ...
         if(xing.equals(new String(tmp))) {
            ...
            byte vbri = 4;
            byte[] e = new byte[4];
            System.arraycopy(firstframe, offset1 + vbri, e, 0, e.length);
            int vbri1 = vbri + e.length;
            if((e[3] & 1) != 0) {
               System.arraycopy(firstframe, offset1 + vbri1, tmp, 0, tmp.length);
            ...
            if((e[3] & 4) != 0) {
               System.arraycopy(firstframe, offset1 + vbri1, this.h_vbr_toc, 0, this.h_vbr_toc.length);
               vbri1 += this.h_vbr_toc.length;
            }
      ...
      try {
         System.arraycopy(firstframe, offset1, tmp, 0, 4);
         if(vbri2.equals(new String(tmp))) {
            this.h_vbr = true;
            this.h_vbr_frames = -1;
            this.h_vbr_bytes = -1;
            this.h_vbr_scale = -1;
            this.h_vbr_toc = new byte[100];
            byte e1 = 10;
            System.arraycopy(firstframe, offset1 + e1, tmp, 0, tmp.length);
            this.h_vbr_bytes = tmp[0] << 24 & -16777216 | tmp[1] << 16 & 16711680 | tmp[2] << 8 & '\uff00' | tmp[3] & 255;
            int e2 = e1 + 4;
            System.arraycopy(firstframe, offset1 + e2, tmp, 0, tmp.length);
            this.h_vbr_frames = tmp[0] << 24 & -16777216 | tmp[1] << 16 & 16711680 | tmp[2] << 8 & '\uff00' | tmp[3] & 255;
            e2 += 4;
         }
   ...
   public int max_number_of_frames(int streamsize) {
      return this.h_vbr?this.h_vbr_frames:(this.framesize + 4 - this.h_padding_bit == 0?0:streamsize / (this.framesize + 4 - this.h_padding_bit));
   }

   public int min_number_of_frames(int streamsize) {
      return this.h_vbr?this.h_vbr_frames:(this.framesize + 5 - this.h_padding_bit == 0?0:streamsize / (this.framesize + 5 - this.h_padding_bit));
   }

   public float ms_per_frame() {
      if(!this.h_vbr) {
         float[][] ms_per_frame_array1 = new float[][]{{8.707483F, 8.0F, 12.0F}, {26.12245F, 24.0F, 36.0F}, {26.12245F, 24.0F, 36.0F}};
         return ms_per_frame_array1[this.h_layer - 1][this.h_sample_frequency];
      } else {
         double ms_per_frame_array = this.h_vbr_time_per_frame[this.layer()] / (double)this.frequency();
         if(this.h_version == 0 || this.h_version == 2) {
            ms_per_frame_array /= 2.0D;
         }

         return (float)(ms_per_frame_array * 1000.0D);
      }
   }
