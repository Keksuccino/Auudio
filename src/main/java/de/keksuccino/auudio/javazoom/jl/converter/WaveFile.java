package de.keksuccino.auudio.javazoom.jl.converter;
...
public class WaveFile extends RiffFile {
   ...
   class WaveFormat_Chunk {
      ...
      public WaveFormat_Chunk() {
         this.header.ckID = FourCC("fmt ");
         this.header.ckSize = 16;
      }

      public int VerifyValidity() {
         boolean ret = this.header.ckID == FourCC("fmt ") && (this.data.nChannels == 1 || this.data.nChannels == 2) && this.data.nAvgBytesPerSec == this.data.nChannels * this.data.nSamplesPerSec * this.data.nBitsPerSample / 8 && this.data.nBlockAlign == this.data.nChannels * this.data.nBitsPerSample / 8;
         return ret?1:0;
      }
   }
